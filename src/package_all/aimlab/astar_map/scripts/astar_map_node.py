...
class AStarPlanner:
    def __init__(self):
        ...
        self.path_narrow_topic = rospy.get_param(
            "~path_narrow_topic", "/astar/path_narrow_passage"
        )
        ...
        self.candidate_route_narrow_lookahead_m = max(
            0.5,
            float(
                rospy.get_param(
                    "~candidate_route_narrow_lookahead_m",
                    min(4.0, self.candidate_route_switch_lookahead_m),
                )
            ),
        )
        self.candidate_route_narrow_clearance_margin_m = max(
            0.0,
            float(
                rospy.get_param(
                    "~candidate_route_narrow_clearance_margin_m",
                    0.08,
                )
            ),
        )
        self.candidate_route_narrow_commit_min_hold_s = max(
            self.candidate_route_switch_min_hold_s,
            float(
                rospy.get_param(
                    "~candidate_route_narrow_commit_min_hold_s",
                    max(self.candidate_route_switch_min_hold_s, 4.0),
                )
            ),
        )
        ...
        self.pub_path_narrow = rospy.Publisher(self.path_narrow_topic, Bool, queue_size=10, latch=True)
        ...
        if self.use_drivable_grid_global:
            ...
            rospy.loginfo(
                "[astar] candidate route policy: inner_fanout=%.2fm prefer_passable=%s passable_clearance=%.2fm shorter_margin=%.2fm narrow_lookahead=%.2fm narrow_margin=%.2fm narrow_hold=%.1fs",
                ...
                self.candidate_route_narrow_lookahead_m,
                self.candidate_route_narrow_clearance_margin_m,
                self.candidate_route_narrow_commit_min_hold_s,
            )
            rospy.loginfo(
    ...
    def _publish_candidate_world_paths_messages(self, world_paths, stamp=None, selected_index=0):
        ...
        for idx, world_points in enumerate(world_paths):
            ...
            marker.scale.x = 0.08 if is_selected else 0.065
            marker.color.r = 0.0
            marker.color.g = 0.0
            marker.color.b = 0.0
            marker.color.a = 1.0 if is_selected else 0.90
    ...
    def _publish_path_narrow_state(self, is_narrow, force=False):
        is_narrow = bool(is_narrow)
        if (not force) and self._path_narrow == is_narrow:
            return
        self._path_narrow = is_narrow
        self.pub_path_narrow.publish(Bool(data=is_narrow))
        ...
        for idx, path in enumerate(world_paths):
            ...
            if (
                ...
                centerline_clear = not self._overlay_world_path_centerline_blocked(
                    ...
                    lookahead_m=local_lookahead_m,
                    start_ignore_m=self.candidate_route_switch_start_ignore_m,
                )
                if (
                    centerline_clear
                    and self.candidate_route_narrow_clearance_margin_m > 1e-6
                ):
                    relaxed_clearance_m = (
                        self.candidate_route_switch_passable_clearance_m
                        + self.candidate_route_narrow_clearance_margin_m
                    )
                    relaxed_clear = not self._overlay_world_path_centerline_blocked(
                        overlay_grid,
                        path,
                        self.global_obstacle_overlay_threshold,
                        clearance_m=relaxed_clearance_m,
                        lookahead_m=local_lookahead_m,
                        start_ignore_m=self.candidate_route_switch_start_ignore_m,
                    )
                    narrow = not relaxed_clear
            metrics.append(
                {
                    "index": idx,
                    ...
                    "relaxed_clear": bool(relaxed_clear),
                    "narrow": bool(narrow),
                }
            )
        return metrics

    def _candidate_metric_for_index(self, metrics, index):
        if not metrics:
            return None
        index = int(max(0, min(index, len(metrics) - 1)))
        return metrics[index]

    def _update_active_path_narrow_state(self, metrics, active_index, force=False):
        metric = self._candidate_metric_for_index(metrics, active_index)
        is_narrow = bool(metric and metric.get("passable") and metric.get("narrow"))
        self._publish_path_narrow_state(is_narrow, force=force)
        return is_narrow
    ...
    def select_initial_candidate_world_path(self, world_paths, active_index=0):
        if not world_paths:
            ...
            self._publish_path_narrow_state(False)
            return 0, []
        ...
        if prefer_passable_initial:
            ...
            self._pending_candidate_switch_count = 0
            self._publish_path_blocked_state(False)
            self._update_active_path_narrow_state(
                passage_metrics, preferred_index, force=True
            )
            if self.debug_log_enable:
                rospy.loginfo(
        ...
        if best_index == active_index:
            ...
            self._update_active_path_narrow_state(
                passage_metrics, active_index, force=True
            )
        ...
        if should_select:
            ...
            self._publish_path_blocked_state(False)
            self._update_active_path_narrow_state(
                passage_metrics, best_index, force=True
            )
            if self.debug_log_enable:
                rospy.loginfo(
    ...
    def select_best_candidate_world_path(self, world_paths, active_index=0):
        ...
        active_metric = self._candidate_metric_for_index(passage_metrics, active_index)
        active_passable = bool(active_metric and active_metric.get("passable"))
        active_narrow = bool(active_metric and active_metric.get("narrow"))
        ...
        if active_passable and active_narrow:
            prefer_passable_switch = False
        ...
        active_hold_s = (
            self.candidate_route_narrow_commit_min_hold_s
            if (active_passable and active_narrow)
            else self.candidate_route_switch_min_hold_s
        )
        can_switch = (
            self._last_candidate_switch_s <= 0.0
            or (now - self._last_candidate_switch_s) >= active_hold_s
        )
        should_switch = (
            current_blocked
        ...
        if (
            ...
            self._publish_path_blocked_state(False)
            self._update_active_path_narrow_state(
                passage_metrics, switch_index, force=True
            )
            if self.debug_log_enable:
        ...
        if self.debug_log_enable and current_blocked:
            rospy.loginfo_throttle(
                1.0,
                "[astar] active candidate blocked by pointcloud overlay but holding route (active=%d score=%.1f best=%d score=%.1f pending=%d/%d centerline=%s best_clear=%s narrow=%s hold=%.1fs)",
                active_index,
                current_score,
                best_index,
                best_score,
    ...
    def clear_published_path(self, stamp=None):
        if self._last_path_nodes is None and self._last_world_path_signature is None:
            ...
            self._publish_path_narrow_state(False, force=True)
            return
        if stamp is None:
            stamp = rospy.Time.now()
        empty = Path()
