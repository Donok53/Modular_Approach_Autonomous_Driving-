...
class TebObstacleCloudFilter:
    def __init__(self):
        ...
        self.max_range = max(self.min_range + 0.1, float(rospy.get_param("~max_range", 6.0)))
        self.self_filter_radius_x = max(0.0, float(rospy.get_param("~self_filter_radius_x", 0.38)))
        self.self_filter_radius_y = max(0.0, float(rospy.get_param("~self_filter_radius_y", 0.33)))
        self.voxel_size = max(0.01, float(rospy.get_param("~voxel_size", 0.10)))
        ...
        rospy.loginfo(
            "teb_obstacle_cloud_filter started | in=%s out=%s z=[%.2f, %.2f] self=%.2fx%.2fm voxel=%.2fm range=%.1f..%.1fm",
            self.input_topic,
            self.output_topic,
            self.min_z,
            self.max_z,
