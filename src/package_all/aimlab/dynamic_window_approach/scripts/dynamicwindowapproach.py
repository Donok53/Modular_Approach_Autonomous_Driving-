...
class DWAControl:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # === 회전 속도(|w|)에 따라 선속도 스케일링 ===
            w_abs = abs(u_cmd[1])

            # 이 이하: 직진에 가깝다고 보고 속도 거의 그대로 사용
            w_slow = math.radians(25.0)   # 25deg/s 이하 → 거의 직진
            # 이 이상: 매우 급회전 → 속도 많이 줄이기
            w_hard = math.radians(140.0)  # 140deg/s 이상 → 강한 감속

            # w에 따라 [1.0 ~ turn_speed_scale] 사이로 선속도 스케일링
            # 급회전 시 max_speed의 35%만 허용 (원하는 값으로 조정)
            turn_speed_scale = 0.35

            if w_abs <= w_slow:
                scale = 1.0
            elif w_abs >= w_hard:
                scale = turn_speed_scale
            else:
                # 선형 보간
                r = (w_abs - w_slow) / (w_hard - w_slow)  # 0~1
                # 1.0 -> turn_speed_scale 로 선형 감소
                scale = (1.0 - r) + r * turn_speed_scale

            # 스케일 적용 (부호는 유지)
            u_cmd[0] = math.copysign(
                min(abs(u_cmd[0]), v_cap * scale), u_cmd[0])
