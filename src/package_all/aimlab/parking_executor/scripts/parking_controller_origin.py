...
class BackInParking:
    ...
    def __init__(self):
        ...
        # === 뒤쪽 장애물 감지 (DWA 스타일 rear ROI) ===
        # DWA와 동일하게 기본값을 /ouster/points 로 맞춤
        self.cloud_topic      = rospy.get_param("~pointcloud_topic", "/ouster/points")
        self.stop_distance    = float(rospy.get_param("~stop_distance", 0.28))   # 뒤쪽 반경
        self.stop_width       = float(rospy.get_param("~stop_width", 0.30))      # 총 폭 (|y| <= width/2)
    ...
    def cb_cloud(self, msg: PointCloud2):
        """
        뒤쪽 ROI에서 장애물 감지.
        - 프레임 기준: x<0 (뒤), |y|<=stop_width/2, min_z<=z<=max_z
        - 반경 stop_distance 이내 포인트 있으면 near=True
        """
        try:
            ...
            rospy.loginfo_throttle(1.0,
                                   "[back_in_parking] cloud recv frame=%s",
                                   msg.header.frame_id)
            ...
            if not self.obstacle_blocked and self.obstacle_on_cnt >= self.block_on_count:
                ...
                rospy.logwarn("Obstacle CLOSE (rear): stopping (<= %.2fm)", self.stop_distance)
            elif self.obstacle_blocked and self.obstacle_off_cnt >= self.block_off_count:
                self.obstacle_blocked = False
                rospy.loginfo("Rear obstacle cleared: resuming")
